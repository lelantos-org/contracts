...
/// Owner-managed registry of supported assets. Each `id` (SNARK
/// `publicAssetId`) binds to an ERC20 and a public-amount → base-units
/// `scale`. Add-only: assets can be disabled but never removed; disabled
/// assets block new deposits but stay spendable so notes/escrows can exit.
abstract contract AssetRegistry is Ownable {
    ...
    /// Owner-only single-asset add. Reverts if `id` already registered.
    function addAsset(uint64 id, IERC20 token, uint256 scale) external onlyOwner {
        _addAsset(id, token, scale);
    }

    /// Owner-only flip of an asset's `disabled` flag.
    ...
    /// Raw lookup for the transact hot path; no revert on missing.
    ...
    /// `scale` sits alone in slot 1 and costs a second cold SLOAD.
