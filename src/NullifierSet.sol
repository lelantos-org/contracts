...
/// Packed-bitmap nullifier set. `_spentBuckets[nf >> 8]` holds 256 bits
/// keyed by `nf & 0xff`.
abstract contract NullifierSet {
    ...
    /// Mark `nf` spent in the packed bitmap; revert on double-spend.
    function _consumeNullifier(bytes32 nf) internal {
        uint256 n = uint256(nf);
        uint256 bucketIdx = n >> 8;
        // shift amount on RHS is correct (`value << shift`); lint heuristic false positive
