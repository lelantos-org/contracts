...
contract ReenteringVenue {
    ...
    /// Written from `POOL()`. A storage write succeeds only if the caller
    /// allowed state modification.
    ...
    /// Declared non-view. `IYieldVenue.POOL()` is `view`, so the pool reaches
    /// this through STATICCALL and the write below reverts.
