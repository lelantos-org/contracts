...
///
/// The fuzz and unit suites reach the bitmap through `MASPHarness`, which pulls
/// in Permit2, the verifiers and the whole pool. Symbolic execution explores
/// every reachable path, so the bitmap is isolated here: this harness adds one
/// external entrypoint and no storage of its own.
///
