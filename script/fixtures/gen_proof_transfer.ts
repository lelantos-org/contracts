...
async function main() {
    ...
    const bootstrapCoeffs = flattenTreeUpdateBatch({
        ...
        pairAsset: bootstrap.pairAsset,
        pairPublicIn: bootstrap.pairPublicIn,
    ...
    // Transfer-leg tree_update_batch (N=1, spend). is_deposit=0; cvDeps[0..1]
    // carry the transact_2x2's outCvDep so the on-chain cross-bind passes.
    ...
    const transferCoeffs = flattenTreeUpdateBatch({
        ...
        pairAsset: transfer.pairAsset,
        pairPublicIn: transfer.pairPublicIn,
    ...
    const fixture = {
        ...
        bootstrap: {
            ...
            pairAsset: bootstrap.pairAsset.map((v) => v.toString()),
            pairPublicIn: bootstrap.pairPublicIn.map((v) => v.toString()),
        ...
        transfer: {
            ...
            pairAsset: transfer.pairAsset.map((v) => v.toString()),
            pairPublicIn: transfer.pairPublicIn.map((v) => v.toString()),
